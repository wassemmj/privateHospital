const {NurseWorkingTime} = require('../../models/medical_models/NurseWorkingTime');
...
module.exports.workingTime = async (req, res) => {
    ...
    const nurseWorkingTimes = await knex('nurseWorkingTimes').where('nurseID', req.params.id);

    if (nurseWorkingTimes) {
        await knex('nurseWorkingTimes').where('nurseID', req.params.id).delete();
    }
    try {
        for (let i = 0; i < newWorkingTime.workingTime.length; i++) {
            ...
            const working = new NurseWorkingTime(data);
            await working.save();
        }
...
module.exports.getWorkingTime = async (req, res) => {
    try {
        const workingTime = await knex('nurseWorkingTimes').where('nurseID', req.params.id);
...
module.exports.getAllNurseTime = async (req, res) => {
    try {
        const times = await knex('nurseWorkingTimes');
